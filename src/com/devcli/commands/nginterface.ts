...
const CMD_DEFAULT = "ng g interface ";

export class NgInterface implements ICommand {
  private static utils: Utils;

  public constructor() {
    NgInterface.utils = new Utils();
  }

  public regMenuCommand(): Disposable {
    // For Context Menu
    let disposable = vscode.commands.registerCommand(
      "angularcliextension.menu_interface",
      args => {
        NgInterface.utils.getRelativePath(args).subscribe(
          v => {
            if (v[0] === "1-ERROR-") return;
            vscode.window
              .showInputBox({
                placeHolder: 'name of interface (relative to "' + v[0] + '"'
              })
              .then(data => {
                if (data != undefined) {
                  let terminal: vscode.Terminal = NgInterface.utils.getTerminalForRootFolder(
                    v[1]
                  );
    ...
    let disposable = vscode.commands.registerCommand(
      ...
      args => {
        ...
        selectedWorkspaceFolder.then(
          function(value) {
            vscode.window
              .showInputBox({ placeHolder: "name of interface" })
              .then(data => {
                if (data != undefined) {
